...
const getSingleUserController = asyncErrorWrapper(async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
...
const deleteUserController = asyncErrorWrapper(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  try {

...
const addNewGameController = expressAsyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const game = await gameService.addNewGameService(req.body, req.user?.id || '');
...
const editGameController = expressAsyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  try {
    const game = await gameService.editGameService(req.body, id, req.user?.id || '');
    res.status(200).json(createResponse(game));
  } catch (error) {
    res.status(404).json(createResponse(null, false, `Error: ${error}`));
  }
...
const getUserGamesController = expressAsyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
const getUserGameDetailController = expressAsyncHandler(async (req: Request, res: Response) => {
  try {
...
const setFavoriteGamesController = expressAsyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      const game = await gameService.setFavoriteGamesService(req.user?.id || '', [
        first_game,
        second_game,
        third_game,
      ]);
